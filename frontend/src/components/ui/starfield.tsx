...
const Starfield = ({
  ...
  const compSpeed =
    hyperspace || state.hyperspace ? speed * warpFactor : speed;
  ...
  const resize = () => {
    ...
    if (
      sd.current.cw !== sd.current.w ||
      sd.current.ch !== sd.current.h
    ) {
      sd.current.x = Math.round(sd.current.w / 2);
      sd.current.y = Math.round(sd.current.h / 2);
      sd.current.z = (sd.current.w + sd.current.h) / 2;
  ...
  const tiltHandler = (event: DeviceOrientationEvent) => {
    if (event.beta !== null && event.gamma !== null) {
      const x = event.gamma;
      const y = event.beta;
      cursor.current.x = sd.current.w / 2 + (x || 0) * 5;
      cursor.current.y = sd.current.h / 2 + (y || 0) * 5;
    }
  };
